...
//		if (i >= n_bits)
//			throw new IndexOutOfBoundsException("Index: " + i + ", Size: " + n_bits);
...
//		if (index >= n_bits)
//			throw new IndexOutOfBoundsException("Index: " + index + ", Size: " + n_bits);
	...
	public void crossover(BinaryGtype g) {
		...
		bytes[wholeBytes]		|= gt.bytes[wholeBytes] & ((1 << local_i) - 1);

		gt.bytes[wholeBytes]	&= ~0 << local_i;
		gt.bytes[wholeBytes]	|= buff[wholeBytes]		& ((1 << local_i) - 1);
	}
	...
	public void mutate(int mode, double prob) {
		switch (mode) {
			case Config.MUTATION_PER_COMPONENT:
				for (int i = 0; i < n_bits; i++) {
					if (Util.randomEvent(prob)) toggleBit(i);
				}
				break;
