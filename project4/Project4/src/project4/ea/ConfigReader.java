...
public class ConfigReader {
	...
	public ConfigReader(String filename) throws IOException {
		...
		if (cfgFile.exists()) {
			...
			prop.setProperty("annConfig", "5,2,2");
			prop.setProperty("elitism", "1");
			prop.setProperty("fitnessTestScenarios", "1");
			prop.setProperty("scenario", "standard");
	...
	public void readEAConfig(Config cfg) {
		...
		p = prop.getProperty("scenario");
		switch (p) {
			case "STANDARD": cfg.scenario = SCENARIO_STANDARD; break;
			case "PULL": cfg.scenario = SCENARIO_PULL; break;
			case "NOWRAP": cfg.scenario = SCENARIO_NOWRAP; break;
			default: throw new ConfigException("scenario", p);
		}
		...
		p = prop.getProperty("annConfig");
		try {
			String[] annConfStr = p.split(",");
			cfg.annConfig = new int[annConfStr.length];

			for (int i = 0; i < cfg.annConfig.length; i++)
				cfg.annConfig[i] = Integer.parseInt(annConfStr[i]);
			...
			throw new RuntimeException("Invalid ann configuration. " + e.getMessage());
		}
		
		cfg.annNumComponents = NeuralNetwork.getNumComponentsForConfig(cfg.annConfig);
		cfg.annNumGains = NeuralNetwork.getNumGainsForConfig(cfg.annConfig);
		cfg.annNumTaus = NeuralNetwork.getNumTausForConfig(cfg.annConfig);
		cfg.annNumWeights = NeuralNetwork.getNumWeightsForConfig(cfg.annConfig);
		cfg.annNumBiases = NeuralNetwork.getNumBiasesForConfig(cfg.annConfig);
	}
